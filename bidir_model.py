...
def bidirectional_conv_lstm_attention_bottleneck_model(
	...
	att_upsample = tf.concat([prev_conv, after_conv], axis = -1)
	...
	for i in range(num_unet_sections):
		...
		att_upsample = tf.keras.layers.Conv2DTranspose(
			d_model,
	...
	final_conv_prediction = tf.keras.layers.Conv2DTranspose(3, unet_upsample_kernel, activation = prediction_activation, padding = "same")(att_upsample)
...
def bidir_model_build(hp):
	...
	# overall parameters
	# conv_attention_or_conv = hp.Choice("CONV_ATTENTION_OR_CONV", ["conv", "attention"])
